...
async function searchCategory() {
  const searchInput = document.getElementById("search-input").value.trim();
      ...
      try {
          const response = await fetch(`/tim-kiem-thu-nhap?name=${encodeURIComponent(searchInput)}`);
          if (response.ok) {
...
function submitCategory() {
  ...
  const idNhom = "1"; // Mặc định nhóm Thu Nhập
  fetch('/addThuNhap', {
    method: 'POST',
    headers: {
...
document.addEventListener("DOMContentLoaded", function () {
  const deleteButtons = document.querySelectorAll(".category-icon-delete");

  deleteButtons.forEach((button) => {
      button.addEventListener("click", function (e) {
          e.preventDefault();

          // Lấy ID của hạng mục cần xóa
          ...
          if (confirm("Bạn có chắc chắn muốn xóa hạng mục này?")) {
              ...
              fetch('/delete-category', {
                  ...
                  body: JSON.stringify({ idHangmuc: categoryId })
              })
...
function updateCategoryList() {
  // Lấy danh sách hạng mục từ Firebase
  const db = firebase.firestore();
  db.collection("HangMuc").get().then((querySnapshot) => {
      const categoryListElement = document.querySelector(".category-list");
      ...
      querySnapshot.forEach((doc) => {
          ...
          categoryListElement.innerHTML += `
              <li class="category-item">
                  ...
                  <div class="category-icon">
                      <a href="#" class="category-icon-edit"><i class="fa-solid fa-pen-to-square" style="color: #000000;"></i></a>
                      <a href="#" class="category-icon-delete" data-id="${doc.id}">
                          <i class="fa-solid fa-trash-can" style="color: #000000;"></i>
                      </a>                            
                  </div>
              </li>`;
